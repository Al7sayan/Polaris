...
namespace polaris
{
    ...
    {
        gpPlayerController->CheatManager->Summon(TEXT("PlayerPawn_Generic_C"));
        m_pPawnActor = static_cast<SDK::APlayerPawn_Generic_C*>(SDKUtils::FindActor(SDK::APlayerPawn_Generic_C::StaticClass()));
        gpPlayerController->Possess(m_pPawnActor);

        // Load Ramirez onto the pawn and replicate character parts.
        auto playerState = static_cast<SDK::AFortPlayerStateZone*>(gpPlayerController->PlayerState);
    ...
    void FarmsteadPawn::Update()
    {
        ...
        bool wantsToSprint = static_cast<SDK::AFortPlayerControllerZone*>(gpPlayerController)->bWantsToSprint;
